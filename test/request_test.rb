...
class RequestTest < Minitest::Test
  ...
  it 'raises Muren::BadRequest when multipart/form-data request has no content' do
    request = Muren::Request.new(
      'REQUEST_METHOD' => 'POST',
      'CONTENT_TYPE' => 'multipart/form-data; boundary=dummy',
      'rack.input' => StringIO.new('')
    )
    assert_raises(Muren::BadRequest) { request.params }
  end

  it 'is secure when the url scheme is https' do
    request = Muren::Request.new('rack.url_scheme' => 'https')
    assert request.secure?
  end

  it 'is not secure when the url scheme is http' do
    request = Muren::Request.new('rack.url_scheme' => 'http')
    assert !request.secure?
  end

  it 'respects X-Forwarded-Host header' do
    request = Muren::Request.new('HTTP_X_FORWARDED_HOST' => 'example.com')
    assert request.forwarded?
  end

  it 'respects Forwarded header with host key' do
    request = Muren::Request.new('HTTP_FORWARDED' => 'host=example.com')
    assert request.forwarded?

    request = Muren::Request.new('HTTP_FORWARDED' => 'for=192.0.2.60;proto=http;by=203.0.113.43')
    refute request.forwarded?
  end

  it 'respects X-Forwarded-Proto header for proxied SSL' do
    request = Muren::Request.new('HTTP_X_FORWARDED_PROTO' => 'https')
    assert request.secure?
  end
  ...
  it 'is possible to marshal params' do
    ...
    Muren::IndifferentHash[request.params]
  ...
  it "raises Muren::BadRequest when params contain conflicting types" do
    request = Muren::Request.new 'QUERY_STRING' => 'foo=&foo[]='
    assert_raises(Muren::BadRequest) { request.params }
  end

  it "makes accept types behave like strings" do
    request = Muren::Request.new('HTTP_ACCEPT' => 'image/jpeg; compress=0.25')
  ...
  it "accepts types when wildcards are requested" do
    request = Muren::Request.new('HTTP_ACCEPT' => 'image/*')
    assert request.accept?('image/jpeg')
  end
  ...
  it 'accepts */* when HTTP_ACCEPT is not present in the request' do
    request = Muren::Request.new Hash.new
  ...
  it 'accepts */* when HTTP_ACCEPT is blank in the request' do
    request = Muren::Request.new 'HTTP_ACCEPT' => ''
    assert_equal 1, request.accept.size
    assert request.accept?('text/html')
    assert_equal '*/*', request.preferred_type.to_s
    assert_equal '*/*', request.preferred_type.to_s(true)
  ...
  it 'will not accept types not specified in HTTP_ACCEPT when HTTP_ACCEPT is provided' do
    request = Muren::Request.new 'HTTP_ACCEPT' => 'application/json'
    assert !request.accept?('text/html')
  end

  it 'will accept types that fulfill HTTP_ACCEPT parameters' do
    request = Muren::Request.new 'HTTP_ACCEPT' => 'application/rss+xml; version="http://purl.org/rss/1.0/"'
  ...
  it 'will accept more generic types that include HTTP_ACCEPT parameters' do
    request = Muren::Request.new 'HTTP_ACCEPT' => 'application/rss+xml; charset=utf-8; version="http://purl.org/rss/1.0/"'
