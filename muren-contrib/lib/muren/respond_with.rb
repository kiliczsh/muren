...
module Muren
  ...
  # = Muren::RespondWith
  #
  # These extensions let Muren automatically choose what template to render or
  # action to perform depending on the request's Accept header.
  #
  # Example:
  #
  #   # Without Muren::RespondWith
  #   get '/' do
  #     data = { :name => 'example' }
  ...
  # * If a file extension associated with the mime type is known to Muren, and
  ...
  module RespondWith
    ...
    def rendering_method(engine)
      return [engine] if Muren::Templates.method_defined? engine
    ...
    def self.engines
      engines = {
        ...
        html: %i[erb erubi haml hamlit slim liquid
                 mab markdown rdoc],
        all: (Muren::Templates.instance_methods.map(&:to_sym) +
