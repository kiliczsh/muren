...
module Muren
  ...
  # Helper methods to ease testing your Muren application. Partly extracted
  # from Muren. Testing framework agnostic.
  module TestHelpers
    ...
    # @!scope class
    # @yieldparam _self [Muren::Base] the object that the method was called on
    #
    # Set configuration options for Muren and/or the app. Allows scoping of
    ...
    # Instantiate and configure a mock Muren app.
    #
    # Takes a `base` app class, or defaults to Muren::Base, and instantiates
    # an app instance. Any given code in `block` is `class_eval`'d on this new
    # instance before the instance is returned.
    #
    # @param base [Muren::Base] App base class
    #
    # @return [Muren] Configured mocked app
    def mock_app(base = Muren::Base, &block)
      inner = nil
      @app  = Muren.new(base) do
        inner = self
        class_eval(&block)
      end
      @settings = inner
    ...
    #
    # @param base [Muren::Base] a configured app
    def app=(base)
      @app = base
    end
    ...
    # Returns a Rack::Lint-wrapped Muren app.
    #
    # If no app has been configured, a new subclass of Muren::Base will be
    # used and stored.
    #
