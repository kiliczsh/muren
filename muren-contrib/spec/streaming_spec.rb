...
RSpec.describe Muren::Streaming do
  def stream(&block)
    rack_middleware = @use
    out = nil
    mock_app do
      rack_middleware.each { |args| use(*args) }
      helpers Muren::Streaming
  ...
  context 'stream test helper' do
    ...
    it 'returns the stream object' do
      out = stream {}
      expect(out).to be_a(Muren::Helpers::Stream)
    end
  ...
  context Muren::Streaming::Stream do
    it 'should extend the stream object' do
      out = stream {}
      expect(out).to be_a(Muren::Streaming::Stream)
    end
    ...
    it 'should not extend stream objects of other apps' do
      ...
      get('/')
      expect(out).to be_a(Muren::Helpers::Stream)
      expect(out).not_to be_a(Muren::Streaming::Stream)
    end
  end
  ...
  context 'app' do
    it 'is the app instance the stream was created from' do
      out = stream {}
      expect(out.app).to be_a(Muren::Base)
    end
  end
