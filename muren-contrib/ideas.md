...
* `muren-smart-cache`: update cache header only if arguments are more
...
* Form helpers, with forms as first class objects that accepts hashes or
  ...
  similar, possibly defining routes (like "Müren's Hat"), strictly using
  the ActiveModel API.

* Extend `muren-content-for` to support Liquid, Radius, Markaby, Nokogiri and
  Builder. At least the first two probably involve patching Tilt.

* Rewrite of `muren-compass`?
